...
class APIService {
    ...
    //temporaire
    private let mistralAPIKey = "jSfN81TqSxFaFyCjj4eON19uS02UJRdw"
    
    private let baseURL = "https://api.mistral.ai/v1/chat/completions" // URL de l'API Mistral

    
    //récupère la clé API stocker dans Info.plist
    //marche pas pour l'instnat
    private var apiKey: String? {
        return Bundle.main.object(forInfoDictionaryKey: "mistralAPIKey") as? String
    }
    
    
    func envoyerMessage(userId: String, question: String, documents: [String], conversation: Conversation, completion: @escaping (APIResponse?) -> Void) {
        ...
        request.setValue("Bearer \(mistralAPIKey)", forHTTPHeaderField: "Authorization") //ajout du jeton d'authentification
